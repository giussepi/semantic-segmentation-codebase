...
from lib.net.operators import ASPP
...
class deeplabv3plus(nn.Module):
    def __init__(self, cfg, batchnorm=nn.BatchNorm2d, **kwargs):
        ...
        self.backbone = build_backbone(
            cfg.MODEL_BACKBONE, pretrained=cfg.MODEL_BACKBONE_PRETRAIN, norm_layer=self.batchnorm, **kwargs)
...
class deeplabv3plusAux(deeplabv3plus):
    ...
    def orth_init(self):
        self.cls_conv.weight = torch.nn.Parameter(
            torch.eye(n=self.cfg.MODEL_NUM_CLASSES, m=self.cfg.MODEL_ASPP_OUTDIM).unsqueeze(-1).unsqueeze(-1))
        self.seghead2[-1].weight = torch.nn.Parameter(torch.eye(n=self.cfg.MODEL_NUM_CLASSES,
                                                      m=self.cfg.MODEL_ASPP_OUTDIM).unsqueeze(-1).unsqueeze(-1))
        self.seghead3[-1].weight = torch.nn.Parameter(torch.eye(n=self.cfg.MODEL_NUM_CLASSES,
                                                      m=self.cfg.MODEL_ASPP_OUTDIM).unsqueeze(-1).unsqueeze(-1))
        self.seghead4[-1].weight = torch.nn.Parameter(torch.eye(n=self.cfg.MODEL_NUM_CLASSES,
                                                      m=self.cfg.MODEL_ASPP_OUTDIM).unsqueeze(-1).unsqueeze(-1))
