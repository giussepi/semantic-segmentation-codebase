...
model_urls = {
    ...
    'xception': 'checkpoints/xception_pytorch_imagenet.pth'
}
...
class Block(nn.Module):
    def __init__(self, in_filters, out_filters, strides=1, atrous=None, grow_first=True, activate_first=True, inplace=True, norm_layer=nn.BatchNorm2d):
        ...
        if grow_first:
            ...
            filters = in_filters
        self.sepconv1 = SeparableConv2d(in_filters, filters, 3, stride=1, padding=1 *
                                        atrous[0], dilation=atrous[0], bias=False, activate_first=activate_first, inplace=self.head_relu, norm_layer=self.norm_layer)
        self.sepconv2 = SeparableConv2d(filters, out_filters, 3, stride=1, padding=1 *
                                        atrous[1], dilation=atrous[1], bias=False, activate_first=activate_first, norm_layer=self.norm_layer)
        self.sepconv3 = SeparableConv2d(out_filters, out_filters, 3, stride=strides, padding=1 *
                                        atrous[2], dilation=atrous[2], bias=False, activate_first=activate_first, inplace=inplace, norm_layer=self.norm_layer)
...
class Xception(nn.Module):
    ...
    def __init__(self, os, norm_layer=nn.BatchNorm2d):
        ...
        self.conv3 = SeparableConv2d(1024, 1536, 3, 1, 1*rate, dilation=rate,
                                     activate_first=False, norm_layer=self.norm_layer)
